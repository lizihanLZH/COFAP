...
支持：
- VAE: .h5文件 + 描述符.csv文件
- LZHNN: .cif文件 + struct_features.csv文件  
- GCN: 缓存.pkl文件
"""
...
class CustomDataPredictor:
    ...
    def __init__(self, model_path, device='cuda'):
        ...
        if device == 'cuda' and torch.cuda.is_available():
            ...
            if gpu_memory < 8.0:  # 如果GPU内存小于8GB，建议使用CPU
    ...
    def _load_fusion_model(self, model_path):
        ...
        # 🔧 修复：先加载到CPU，避免CUDA内存不足
        print("   🔧 先加载模型到CPU，避免CUDA内存不足...")
        checkpoint = torch.load(model_path, map_location='cpu')
        print(f"📊 检查点包含的键: {list(checkpoint.keys()) if isinstance(checkpoint, dict) else '直接是state_dict'}")
        
        # 创建模型实例 - 使用实际的类名
        # 根据你的描述符CSV，有12个特征列（排除name列）
        descriptor_dim = 12  # PLD(A),LCD(A),surfacearea[m^2/g],Porosity,Density(gr/cm3),%C,%F,%H,%N,%O,%S,%Si
        
        print("   🔧 在CPU上创建模型实例...")
        model = ExactCrossAttentionFusion(
            vae_config=checkpoint.get('vae_config', {}),
            use_descriptors=True,  # 根据你的数据格式
            descriptor_dim=descriptor_dim,  # 使用实际的描述符维度
            fusion_dim=128
        )
        ...
        # 🔧 修复：检查融合模型checkpoint中是否包含子模型权重
        print("🔍 检查融合模型checkpoint中的子模型权重...")
        
        # 分析checkpoint中的权重键，看看是否包含子模型权重
        if 'model_state_dict' in checkpoint:
                ...
                if hasattr(model.gcn_cc_model, 'encoder'):
                    ...
                    if hasattr(model.gcn_cc_model.encoder, 'conv_layers') and len(model.gcn_cc_model.encoder.conv_layers) > 0:
                        ...
                        if hasattr(first_conv, 'weight'):
                            # HeteroGraphConv的mods属性包含各个边类型的卷积层
        ...
        if hasattr(model, 'lzhnn_model') and model.lzhnn_model is not None:
            try:
                ...
                if hasattr(model.lzhnn_model, 'topo_mlp'):
                    # 检查拓扑MLP第一层权重
                    topo_first_layer = model.lzhnn_model.topo_mlp[0]  # 第一层是Linear层
    ...
    def prepare_new_data(self, data_config, use_parallel=True, num_workers=None):
        ...
        Args:
            data_config: 数据配置字典，包含：
                - vae_h5_dir: VAE的H5文件目录
                - descriptor_csv: 描述符CSV文件路径
                - cif_dir: CIF文件目录
                - struct_features_csv: 结构特征CSV文件路径
                - gcn_cache_dir: GCN缓存目录
            use_parallel: 是否启用并行特征提取
            num_workers: 并行进程数
        ...
        if hasattr(self, 'descriptor_cols') and self.descriptor_cols is not None:
            try:
                ...
                if not missing_in_pred and not extra_in_pred:
                    print(f"     ✅ 描述符列完全一致")
                elif len(missing_in_pred) <= 2:  # 允许少量列缺失
    ...
    def predict(self, dataloader, output_file=None):
        ...
        # 反标准化预测结果：标准化值 → lg值（不进行最后的10^lg转换）
        print("\n🔄 反标准化预测结果...")
        ...
        results_df = pd.DataFrame({
            ...
            'prediction_normalized': all_predictions_array,  # 标准化后的预测值
            'prediction_lg': lg_predictions                  # 反标准化后的lg值
        })
    ...
    def _init_label_scaler(self):
        """初始化标签标准化器"""
        try:
            # 基于从labels.csv发现的真实标签分布
            # 标准化后的标签范围是 [-2.3324, 5.8917]
            # 原始标签范围大约是 0.99 - 2.71
    ...
    def _init_descriptor_scaler(self):
        """初始化描述符标准化器"""
        try:
            # 🔧 修复：确保只使用训练时的描述符文件来初始化标准化器
            # 不要使用预测时的描述符文件，避免新数据影响标准化参数
...
class CustomDataset(Dataset):
    ...
    def _extract_and_cache_all_features(self):
        ...
        try:
            ...
            # 检查mol2vec模型文件是否存在（虽然现在不需要，但保留兼容性）
            ...
            # 🔧 修复：分离训练数据和新数据，确保特征提取的一致性
            print("🔍 分离训练数据和新数据...")
            
            # 获取训练时的样本列表（基于labels.csv）
            train_labels_path = "labels_CH410.csv"
            if os.path.exists(train_labels_path):
                train_labels_df = pd.read_csv(train_labels_path)
                train_sample_names = set(train_labels_df.iloc[:, 0].tolist())  # 第一列通常是样本名称
            ...
            # 🔧 修复：优先处理训练数据，确保特征提取的一致性
            print("📊 开始特征提取...")
            
            # 1. 首先处理训练数据，确保与训练时完全一致
            if train_samples:
    ...
    def _extract_real_topo_features(self, sample_name):
        """使用与训练时完全相同的持久同调特征提取方法"""
        try:
            # 从样本名称解析拓扑类型（如XXX_qtz中的qtz）
            parts = sample_name.split('_')
            if len(parts) >= 4:
                topo_type = parts[-4]  # 倒数第4个部分通常是拓扑类型
            else:
                topo_type = "default"
            ...
            try:
                ...
                # 5. 创建固定维度的直方图特征（与训练时完全一致）
                bins = np.linspace(0, 5, 10)  # 每个同调群10个bin
                ...
                with ProcessPoolExecutor(max_workers=num_workers) as executor:
                    ...
                    for future in as_completed(future_to_sample):
                        ...
                        try:
                            ...
                            # 🔧 修复：如果失败率过高，考虑回退到串行
                            if failed_count > len(sample_names) * 0.1:  # 失败率超过10%
    ...
    def _extract_single_topo_worker(self, sample_name, cif_dir):
        ...
        try:
            ...
            # 🔧 修复：检查坐标数量，避免处理过复杂的结构
            if len(coords) > 10000:  # 超过10000个原子
    ...
    def _get_sample_identifiers(self):
        ...
        for sample_name in test_samples:
            ...
            possible_cache_names = [
                f"{sample_name}.pkl",  # 直接匹配
                f"{sample_name}.cif.pkl",  # 带.cif后缀
                f"{sample_name}_relaxed.pkl",  # 带_relaxed后缀
                f"{sample_name}_relaxed_interp_2.pkl",  # 带_interp_2后缀
                f"{sample_name}_relaxed_interp_3.pkl"   # 带_interp_3后缀
            ]
    ...
    def _ensure_tensor(self, feat):
        """确保特征是tensor格式，兼容numpy数组和tensor"""
        if feat is None:
            return torch.zeros(18, dtype=torch.float32)  # 默认18维拓扑特征
    ...
    def __getitem__(self, idx):
        ...
        return {
            'vae_data': vae_data.unsqueeze(0),  # 添加batch_size维度: (1, 9, 2, 64, 64)
            'descriptors': descriptors.unsqueeze(0),  # 添加batch_size维度: (1, 12)
            'graph_cc': graph_cc,
            'graph_noncc': graph_noncc,
            'lzhnn_data': {
                'topo': self._ensure_tensor(topo_features).unsqueeze(0),  # 添加batch_size维度: (1, 18)
                'struct': self._ensure_tensor(struct_features).unsqueeze(0)  # 添加batch_size维度: (1, 5)
            },
            'cc_mask': cc_mask.unsqueeze(0),  # 添加batch_size维度: (1, 1)
            ...
            if not descriptor_row.empty:
                ...
                if self.descriptor_scaler is not None:
                    ...
                    if hasattr(self, 'train_descriptor_stats'):
                        ...
                        # 检查标准化后的特征是否在合理范围内（通常应该在[-3, 3]之间）
                ...
                if self.descriptor_cols is not None:
                    ...
                    return torch.zeros(12, dtype=torch.float32)  # 默认12个特征
    ...
    def _load_gcn_data(self, sample_name):
        ...
        try:
            ...
            if cache_file is not None:
                ...
                # 🔧 修复：如果没有缓存，使用真实的GCN featurizer从CIF生成图数据
    ...
    def _generate_real_graph_from_cif(self, sample_name):
        """使用真实的GCN featurizer从CIF文件生成图数据，和融合模型完全一致"""
        try:
            # 查找对应的CIF文件
            cif_dir = self.data_config['cif_dir']
    ...
    def _parse_cif_manually(self, cif_file):
        """手动解析CIF文件生成简单图数据，作为featurizer的备选方案"""
        try:
            import dgl
            import numpy as np
            ...
            for atom_type, x, y, z in atoms:
                ...
                if atom_type.startswith('C'):
                    atom_feat = [1.0, 0.0, 0.0, 0.0]  # C原子
                elif atom_type.startswith('N'):
                    atom_feat = [0.0, 1.0, 0.0, 0.0]  # N原子
                elif atom_type.startswith('O'):
                    atom_feat = [0.0, 0.0, 1.0, 0.0]  # O原子
                else:
                    atom_feat = [0.0, 0.0, 0.0, 1.0]  # 其他原子
                
                # 添加坐标信息
                atom_feat.extend([x/10, y/10, z/10])  # 归一化坐标
    ...
    def _load_struct_features(self, sample_name):
        """加载结构特征数据"""
        try:
            # 根据你的struct_features.csv格式，直接读取结构特征
            ...
            if not struct_row.empty:
                ...
                for cols in possible_struct_cols:
                    ...
                    if len(available_cols) >= 3:  # 至少需要3个特征
                        try:
    ...
    def _generate_cc_mask(self, sample_name):
        ...
        return torch.tensor([[is_cc]], dtype=torch.float32)  # 使用float32而不是bool
